...
const OrbitingBoxes = () => {
  ...
  return (
    <>
      <Box scale={1} position={[r * Math.cos(t), r * Math.sin(t), 0]} color="#FF5555" />
      <Box
        scale={1}
        position={[1.2 * r * Math.cos(t * 1.2 + (Math.PI / 3) * 2), 1.5 * r * Math.sin(t + (Math.PI / 3) * 2), 0]}
        color="#55BB55"
      />
      <Box
        scale={1}
        position={[3 * r * Math.cos(t + (Math.PI / 3) * 2 * 2), 0.5 * r * Math.sin(t * 2 + (Math.PI / 3) * 2 * 2), 0]}
        color="#5555FF"
      />
    </>
  )
...
const Clouds = () => {
  const [billboardTexture, alpha] = useLoader(TextureLoader, ['assets/ps2_clouds.png'])
    ...
    <div>
      <Suspense fallback={null}>
        ...
        <Box position={[1.2, 0, 0]} /> */}
          ...
          <Sphere color="#0000ff" phase={Math.PI + Math.PI / 3} a={0.02} b={0.3} c={0.0004} />
          <Clouds />
          <Effects />
        </Canvas>
      </Suspense>
